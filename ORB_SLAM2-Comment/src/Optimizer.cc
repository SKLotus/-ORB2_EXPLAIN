	...
	int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
	...
	vbNotIncludedMP.resize(vpMP.size());//通过调用resize函数，vbNotIncludedMP的大小被设置为与vpMP相同。这样，现在vbNotIncludedMP的每个元素默认都是false，表示所有地图点都尚未被包含在集合中。

	// Step 1 初始化g2o优化器
	g2o::SparseOptimizer optimizer;//首先，创建了一个g2o::SparseOptimizer对象optimizer，用于执行稀疏优化。
	g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
	linearSolver = new g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>();//linearSolver被声明为g2o::BlockSolver_6_3::LinearSolverType*类型的指针，并且通过new关键字创建了一个g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>对象，并将其地址赋给了linearSolver。
	g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(linearSolver);//solver_ptr被声明为g2o::BlockSolver_6_3*类型的指针，并通过new关键字创建了一个g2o::BlockSolver_6_3对象，并将之前创建的线性求解器对象linearSolver作为参数传递给了构造函数。
	// 使用LM算法优化
	...
	for (size_t i = 0; i < vpKFs.size(); i++)
	{
		...
		vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));//Converter::toSE3Quat(pKF->GetPose())返回了一个表示位姿的类型，可能是四元数或其他表示方式，然后使用setEstimate方法将其作为估计值设置给了vSE3对象。该估计值用于图优化过程中对顶点的优化。
	...
	for (size_t i = 0; i < vpMP.size(); i++)
	{
		...
		for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++)
		{
			...
			if (pKF->mvuRight[mit->second] < 0)
			{
				...
				e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);//创建一个2x2的单位矩阵，并将其乘以 invSigma2。这样就得到了一个2x2的矩阵，其对角线元素值为 invSigma2，表示边的信息矩阵。通过 e->setInformation() 将计算得到的信息矩阵设置给边 e
				// 使用鲁棒核函数
				if (bRobust)
				{
	...
	for (size_t i = 0; i < vpKFs.size(); i++)
	{
		...
		// 获取到优化后的位姿
		/*
		首先，使用optimizer.vertex(pKF->mnId)获取与关键帧pKF对应的顶点vSE3。mnId可能是pKF的唯一标识符。

        然后，通过static_cast<g2o::VertexSE3Expmap*>(...)将获取的顶点转换为g2o库中的VertexSE3Expmap类型。这是一个表示带有指数映射的3D位姿的顶点。

        最后，通过vSE3->estimate()获取顶点vSE3的位姿估计值。该位姿估计由g2o库提供的SE3Quat类型表示，它是一个四元数和位移向量的组合，用于表示3D刚体的位姿。
		*/
		...
		else
		{
			...
			pKF->mnBAGlobalForKF = nLoopKF;//看上去是闭环关键帧的ID，调用实际是当前关键帧的ID
		}
	}
		...
		for (int i = 0; i < N; i++)
		{
			...
			if (pMP)
			{
				...
				if (pFrame->mvuRight[i] < 0)
				{
					...
					// 对这个地图点的观测
					Eigen::Matrix<double, 2, 1> obs;//创建一个2x1大小的Eigen矩阵obs，最终，obs矩阵将包含了关键点在图像上的x和y坐标的信息。
					const cv::KeyPoint &kpUn = pFrame->mvKeysUn[i];//
					obs << kpUn.pt.x, kpUn.pt.y;//并按顺序将kpUn.pt.x和kpUn.pt.y的值赋给矩阵的元素。
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		/*
		e12->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));这行代码的作用是将e12这条边的第二个顶点（索引为1）设置为optimizer.vertex(0)对应的顶点。

在这里，dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0))是一个类型转换操作，将optimizer.vertex(0)的返回值从基类指针g2o::HyperGraph::Vertex*转换为派生类指针g2o::OptimizableGraph::Vertex*。这是一个向下转型的操作，通常用于将基类指针转换成派生类指针。

总结一下，这段代码的目的是将图优化问题中的边e12的第二个顶点设置为optimizer中索引为0的顶点。
		*/
