                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
	...
	// 构造线程来计算H矩阵及其得分
    thread threadH(
		&Initializer::FindHomography,//该线程的主函数，即initializer.cc里面的findhomography函数
		this,  //由于主函数为类的成员函数，所以第一个参数就应该是当前对象的this指针
		ref(vbMatchesInliersH), //输出，特征点对的Inlier标记
		ref(SH), //输出，计算单应矩阵的RANSAC评分
		ref(H));//输出，计算的单应矩阵结果
	// thread方法比较特殊，在传递引用的时候，外层需要用ref来进行引用传递，否则就是浅拷贝
    thread threadF(&Initializer::FindFundamental,this,ref(vbMatchesInliersF), ref(SF), ref(F));//以引用的方式调用参数

    // Wait until both threads have finished
	//等到子线程执行完成后，主线程才继续执行，此时主线程会释放掉执行完成后的子线程的资源。
    threadH.join();
    threadF.join();
    ...
    if(RH>0.40)//更偏向于平面，从单应矩阵恢复，函数reconstruction返回bool结果
    ...
    return false;//一般地程序不应该执行到这里，如果执行到这里说明程序跑飞了
}
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        cv::Mat Hn = ComputeH21(vPn1i,vPn2i);//八点法计算参考帧到当前帧的H矩阵
		...
		// Step 4 利用重投影误差为当次RANSAC的结果评分
        currentScore = CheckHomography(H21i, H12i, //输入单应矩阵计算结果
			vbCurrentInliers, //输出特征点对Inliers的标记
			mSigma);//测量误差，在Initializer类对象构造时候，由外部输入的
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    // Iteration variables
	//下面分别对应着某次迭代中参考帧对应的特征点坐标、当前帧对应的特征点坐标、计算的基础矩阵
    vector<cv::Point2f> vPn1i(8);
    vector<cv::Point2f> vPn2i(8);
    cv::Mat F21i;
	//每次RANSAC记录的内点和得分
...
{   
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // Reprojection error in first imag
        ...
        const float squareDist1 = (u1-u2in1)*(u1-u2in1)+(v1-v2in1)*(v1-v2in1);//自由度为2是因为u2in1和v2in1都是可以任意选择的，如果是rgbd，自由度就是3
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
	...
	//Step 2 计算特征点X，Y坐标离均值的平均偏离程度，主义不是标准差
    float meanDevX = 0;
    float meanDevY = 0;

	//用原始特征点的坐标减去均值坐标，得到差值
    for(int i=0; i<N; i++)
    {
	...
	// Step 3 将x坐标和y坐标分别进行尺度归一化，使得x坐标和y坐标的一阶绝对矩分别为1 
	// 这里所谓的一阶绝对矩其实就是随机变量到取值中心的绝对值的平均值（期望）
    for(int i=0; i<N; i++)
    {
		//简单地对特征点的坐标进行进一步的缩放
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        if(!vbMatchesInliers[i])// 跳过outliers
            continue;
