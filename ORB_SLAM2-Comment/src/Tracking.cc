	...
	mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);//参数的含义看上面的注释

	// 如果是双目，tracking过程中还会用用到mpORBextractorRight作为右目特征点提取器
    if(sensor==System::STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);

	// 在单目初始化的时候，会用mpIniORBextractor来作为特征点提取器,与 mpORBextractorLeft、mpORBextractorRight 的不同，主要在于 _nfeatures 参数的不同，也就是特征提取的数目不一样。
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)//处理双目
{
    mImGray = imRectLeft;
    cv::Mat imGrayRight = imRectRight;
	// Step 1 ：转换为灰度图
    if(mImGray.channels()==3)//判断是否是三通道
    {
        if(mbRGB)
        {
    ...
    else if(mImGray.channels()==4)//判断是否是四通道
    {
        if(mbRGB)
        {
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)//处理RGBD
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)
        imDepth.convertTo(imDepth,CV_32F,mDepthMapFactor);//转化为深度因子
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)//处理单目
{
    mImGray = im;
	// Step 1 ：将彩色图像转为灰度图像
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)//没有成功初始化的前一个状态就是NO_IMAGES_YET
        ...
        mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,//正常运行的时的ORB特征点提取器，提取指定数目特征点
			mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
	// Step 3 ：跟踪
    Track();
	//返回当前帧的位姿
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD)
            StereoInitialization();//双目初始化,和RGBD共用一个函数
        else
    ...
    else//如果已经完成初始化
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)//如果跟踪正常
            {
                ...
                else//先用当前帧追踪(恒速模式)，如果追踪失败，则使用关键帧追踪
                {
					// 用最近的普通帧来跟踪当前的普通帧
					// 根据恒速模型设定当前帧的初始位姿
        ...
        else//仅追踪模式
        {
            ...
            else//如果跟踪正常
            {
                ...
                if(!mbVO)//从地图匹配到了很多MapPoints,追踪正常
                {
                    ...
                    if(!mVelocity.empty())//如果恒速模型还有效
                    {
						
                        bOK = TrackWithMotionModel();//先使用恒速模式跟踪然后再进行重定位
                    ...
                    {
                        bOK = TrackReferenceKeyFrame();//恒速模型无效则用参考关键帧来追踪
                    }
                }
                else
                {
        ...
        {
            if(bOK)
                bOK = TrackLocalMap();//对local map进行跟踪得到更多的匹配，并优化当前位姿
        }
        else
        {
    ...
    if(!mCurrentFrame.mTcw.empty())
    {
		...
		//保存各种状态
        mltiveFramePoses.push_back(Tcr);
...
void Tracking::MonocularInitialization()
{
    ...
    else//如果单目初始化器已经被创建
    {
        ...
        // Find correspondences
		 // Step 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
        ORBmatcher matcher(0.9,//最佳和次佳特征点评分的比值阈值，这里较为宽松，跟踪时一般是0.7
			true);//检查特征点方向
